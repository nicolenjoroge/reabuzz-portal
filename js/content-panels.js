...
(function (window) {
  ...
  function toolbar(section) {
    return '<button class="btn btn-ghost btn-sm" onclick="CS.discard()">Discard</button>' +
  ...
  function imagePicker(csPath, current, previewId) {
    return '<div class="field">' +
      ...
      '<div style="display:flex;gap:8px;align-items:center;margin-bottom:6px;">' +
        '<span id="' + previewId + '_path" style="font-size:12px;color:var(--txt2);flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' +
          e(current || 'No image selected') +
        '</span>' +
        '<button class="btn btn-ghost btn-sm" type="button" ' +
          'data-action="pick-image" data-path="' + csPath + '" data-preview="' + previewId + '">' +
          'Choose \u2026' +
        '</button>' +
      '</div>' +
      (current
        ? '<img id="' + previewId + '" src="' + CS.mediaUrl(current) + '" ' +
          'style="width:100%;max-height:140px;object-fit:cover;border-radius:8px;" ' +
          'onerror="this.style.display=\'none\'">'
        : '<div id="' + previewId + '" style="width:100%;height:80px;border:1px dashed var(--border);border-radius:8px;display:flex;align-items:center;justify-content:center;color:var(--txt2);font-size:12px;">No image</div>'
      ) +
    '</div>';
  }
  ...
  function videoPicker(csPath, current, previewId) {
    return '<div class="field">' +
      ...
      '<div style="display:flex;gap:8px;align-items:center;">' +
        ...
        '</span>' +
        '<button class="btn btn-ghost btn-sm" type="button" ' +
          'data-action="pick-video" data-path="' + csPath + '" data-preview="' + previewId + '">' +
          'Choose \u2026' +
        '</button>' +
      '</div>' +
    '</div>';
  ...
  // Delegated listener on drawer body — handles all pick-image / pick-video buttons
  ...
  // Remove old window._pickImage and _pickVideo — replaced by delegated listener above
  ...
  window.renderLanding = function () {
    ...
    var initOpts  = initItems.map(function (it) {
      return '<option value="' + e(it.id) + '"' + (it.id === featRef ? ' selected' : '') + '>' +
        e(it.title) + '</option>';
    }).join('');

    // Marquee rows
    var marqueeRows = marquee.map(function (m, i) {
      return listRow(
        '<strong>' + e(m.value) + '</strong> &nbsp;<span style="color:var(--txt2)">' + e(m.label) + '</span>' +
        (m.highlight ? ' &nbsp;<span class="pill pill-deployed" style="font-size:10px;">Highlight</span>' : ''),
        null,
        '<button class="btn btn-ghost btn-sm" onclick="_landingMarqueeEdit(' + i + ')">Edit</button>' +
        '<button class="btn btn-ghost btn-sm btn-icon" onclick="CS.removeItem(\'landing.marquee\',' + i + ');window.renderLanding()">&#x2715;</button>'
      );
    }).join('');

    // Video refId rows
    var videoRows = videoRefs.map(function (id, i) {
      return listRow(
        e(id),
        null,
    ...
    document.getElementById('mainArea').innerHTML =
      header('Landing page', 'Marquee stats, hero text, featured initiative, video shelf.', toolbar('landing')) +
      '<div class="panel-body">' +

      card('Marquee stats', 'The four rolling numbers at the top of the page.',
        marqueeRows +
        '<button class="add-btn" onclick="_landingMarqueeEdit(-1)">+ Add stat</button>'
      ) +

      card('Hero text', 'landing.hero',
        '<div class="form-grid" style="grid-template-columns:1fr 1fr;gap:14px;">' +
          field('Eyebrow',      input('landing.hero.eyebrow',      hero.eyebrow)) +
          field('Heading line', input('landing.hero.headingLine1', hero.headingLine1)) +
        '</div>'
      ) +

      card('Featured initiative', 'landing.featuredInitiative.refId',
        field('Initiative',
      ...
      card('Video shelf', 'landing.videoSlides.refIds',
        videoRows +
        '<button class="add-btn" onclick="_landingAddVideo()">+ Add refId</button>'
      ) +
  ...
  window._landingMarqueeEdit = function (idx) {
    ...
    document.getElementById('drawerTitle').textContent = isNew ? 'Add stat' : 'Edit stat';
    document.getElementById('drawerSub').textContent   = isNew ? 'New marquee stat' : m.value;
    ...
    document.getElementById('drawerBody').innerHTML =
      '<div class="form-grid" style="grid-template-columns:1fr 1fr;gap:14px;">' +
        field('Value (e.g. 200,000+)', '<input id="mq_val" value="' + e(m.value) + '">') +
        field('Label',                 '<input id="mq_lbl" value="' + e(m.label) + '">') +
        field('Highlight?',
          '<select id="mq_hl">' +
            '<option value="false"' + (!m.highlight ? ' selected' : '') + '>No</option>' +
            '<option value="true"'  + ( m.highlight ? ' selected' : '') + '>Yes</option>' +
          '</select>') +
      '</div>';
  ...
  window._landingAddVideo = function () {
  ...
  window.renderREAStory = function () {
    ...
    document.getElementById('mainArea').innerHTML =
      header('REA Story', 'The two scroll panels — what REA is, and who drives it.', toolbar('reastory')) +
      '<div class="panel-body">' +

      card('Teaser card',
        'The storyhub card that links visitors to this section.',
        '<div class="form-grid" style="grid-template-columns:1fr 1fr;gap:14px;">' +
          field('Section label', input('reaStory.teaser.sectionLabel', teaser.sectionLabel)) +
          field('Headline',      input('reaStory.teaser.headline',     teaser.headline)) +
          field('Link href',     input('reaStory.teaser.href',         teaser.href)) +
        '</div>'
      ) +

      card('Panel 1 — What REA is',
        'reaStory.rea',
        '<div class="form-grid" style="grid-template-columns:1fr 1fr;gap:14px;">' +
          field('Tag',     input('reaStory.rea.tag',     rea.tag)) +
          field('Heading', input('reaStory.rea.heading', rea.heading)) +
          '<div class="field" style="grid-column:span 2">' +
            '<label>Body copy</label>' + textarea('reaStory.rea.body', rea.body, 6) +
          '</div>' +
          videoPicker('reaStory.rea.video', rea.video, 'prev_reavid') +
        '</div>'
      ) +

      card('Panel 2 — Who drives it (BPM)',
        'reaStory.bpm',
        '<div class="form-grid" style="grid-template-columns:1fr 1fr;gap:14px;">' +
          field('Tag',     input('reaStory.bpm.tag',     bpm.tag)) +
          field('Heading', input('reaStory.bpm.heading', bpm.heading)) +
          '<div class="field" style="grid-column:span 2">' +
            '<label>Body copy</label>' + textarea('reaStory.bpm.body', bpm.body, 5) +
          '</div>' +
          imagePicker('reaStory.bpm.image', bpm.image, 'prev_bpm') +
        '</div>'
      ) +
  ...
  window.renderPortfolio = function () {
    ...
    var topRows = items.map(function (item, i) {
      return listRow(
        '<span style="font-size:18px;font-weight:bold;color:var(--txt2);opacity:.25;margin-right:8px;">' + (i + 1) + '</span>' +
        e(item.title),
        e(item.id) + (item.stream ? ' \u00b7 ' + e(item.stream) : '') + (item.businessArea ? ' \u00b7 ' + e(item.businessArea) : ''),
        ...
        '<button class="btn btn-ghost btn-sm btn-icon" onclick="CS.moveItem(\'innovation.topInitiatives.items\',' + i + ',1);window.renderPortfolio()">\u2193</button>' +
        '<button class="btn btn-ghost btn-sm" onclick="_portfolioInitiativeDrawer(' + i + ')">Edit</button>'
      );
    }).join('');
    ...
    var deployedRows = deployed.map(function (item, i) {
      return listRow(
        e(item.title),
        e((item.description || '').substring(0, 80)) + (item.description && item.description.length > 80 ? '\u2026' : ''),
        '<button class="btn btn-ghost btn-sm" onclick="_portfolioDeployedDrawer(' + i + ')">Edit</button>' +
    ...
    var comingRows = coming.map(function (item, i) {
      return listRow(
        e(item.title),
        e((item.body || '').substring(0, 80)) + (item.body && item.body.length > 80 ? '\u2026' : ''),
        '<button class="btn btn-ghost btn-sm" onclick="_portfolioComingSoonDrawer(' + i + ')">Edit</button>' +
    ...
    var videoRows = videos.map(function (v, i) {
      return listRow(
        e(v.title),
        e(v.tag) + ' \u00b7 ' + e(v.id),
        '<button class="btn btn-ghost btn-sm" onclick="_portfolioVideoDrawer(' + i + ')">Edit</button>' +
        '<button class="btn btn-ghost btn-sm btn-icon" onclick="CS.removeItem(\'innovation.videos.items\',' + i + ');window.renderPortfolio()">&#x2715;</button>'
      );
    }).join('');

    document.getElementById('mainArea').innerHTML =
      header('Innovation Portfolio', 'Top initiatives, coming soon, and video highlights.', toolbar('portfolio')) +
      '<div class="panel-body">' +

      card('Section copy', 'innovation.teaser + sectionSub',
        '<div class="form-grid" style="grid-template-columns:1fr 1fr;gap:14px;">' +
          field('Teaser headline',  input('innovation.teaser.headline',  teaser.headline)) +
          field('Section subtitle', input('innovation.sectionSub',       iv.sectionSub)) +
          field('Section label',    input('innovation.teaser.sectionLabel', teaser.sectionLabel)) +
          field('Link href',        input('innovation.teaser.href',      teaser.href)) +
        '</div>'
      ) +

      card('Top initiatives',
        'Reorder with arrows. Edit opens the full story, stats, and media fields.',
        topRows
      ) +

      card('Deployed initiatives',
        'innovation.deployed.items',
        deployedRows +
  ...
  window._portfolioInitiativeDrawer = function (idx) {
    ...
    document.getElementById('drawerBody').innerHTML =
      '<div class="form-grid" style="grid-template-columns:1fr 1fr;gap:12px;">' +
        ...
        '<div class="field" style="grid-column:span 2"><label>Stats</label>' +
          ...
          stats.map(function (s, si) {
            return '<div style="display:flex;gap:6px;">' +
              '<input placeholder="Value" value="' + e(s.value) + '" oninput="CS.update(\'' + base + '.stats.' + si + '.value\',this.value)" style="flex:1">' +
              '<input placeholder="Label" value="' + e(s.label) + '" oninput="CS.update(\'' + base + '.stats.' + si + '.label\',this.value)" style="flex:1">' +
            '</div>';
          }).join('') +
          '</div></div>' +
        ...
        '</div>' +
        field('Story — closing line', input(base + '.story.closing.line', story.closing && story.closing.line)) +
        '<div class="field" style="grid-column:span 2"><label>Story — closing body</label>' +
          textarea(base + '.story.closing.body', story.closing && story.closing.body, 3) +
        '</div>' +

      '</div>';

    document.getElementById('drawerOverlay').className = 'drawer-overlay open';
    _drawerSave = function () { closeDrawer(); window.renderPortfolio(); };
  };
  ...
  window._portfolioDeployedDrawer = function (idx) {
    ...
    document.getElementById('drawerTitle').textContent    = isNew ? 'Add deployed initiative' : 'Edit — ' + item.title;
    ...
    document.getElementById('drawerBody').innerHTML =
      '<div class="form-grid" style="grid-template-columns:1fr;gap:12px;">' +
        ...
        field('Description', '<textarea id="dep_d" rows="4">' + e(item.description) + '</textarea>') +
    ...
    _drawerSave = function () {
      var updated = {
        ...
        image:       CS.get() && (function(){ var a=(CS.get().innovation.deployed||{}).items||[]; return a[idx]&&a[idx].image||''; })(),
      };
      if (isNew) {
  ...
  window._portfolioComingSoonDrawer = function (idx) {
    ...
    document.getElementById('drawerTitle').textContent    = isNew ? 'Add coming soon' : 'Edit — ' + item.title;
    document.getElementById('drawerSub').textContent      = isNew ? '' : item.title;
    document.getElementById('drawerDelete').style.display = isNew ? 'none' : '';
    document.getElementById('drawerDelete').onclick = function () {
    ...
    document.getElementById('drawerBody').innerHTML =
      '<div class="form-grid" style="grid-template-columns:1fr;gap:12px;">' +
        ...
        field('Body',  '<textarea id="cs_b" rows="3">' + e(item.body) + '</textarea>') +
    ...
    _drawerSave = function () {
      var updated = {
        ...
        image: CS.get() && (function(){ var a=(CS.get().innovation.comingSoon||{}).items||[]; return (isNew ? '' : (a[idx]&&a[idx].image||'')); })(),
        description: '',
      };
      if (isNew) {
  ...
  window.renderSpotlight = function () {
    ...
    var testimonialRows = tItems.map(function (t, i) {
      return listRow(
        '\u201c' + e(t.quote.substring(0, 70)) + (t.quote.length > 70 ? '\u2026' : '') + '\u201d',
        e(t.name) + ' \u00b7 ' + e(t.role),
        '<button class="btn btn-ghost btn-sm btn-icon" onclick="_spotlightTestimonialDrawer(' + i + ')">&#x270E;</button>' +
    ...
    var peopleRows = pItems.map(function (p, i) {
      return listRow(
        e(p.name),
        e(p.contribution) + ' \u00b7 <span class="pill pill-deployed" style="font-size:10px;">' + e(p.tag) + '</span>',
        '<button class="btn btn-ghost btn-sm btn-icon" onclick="_spotlightPersonDrawer(' + i + ')">&#x270E;</button>' +
        '<button class="btn btn-ghost btn-sm btn-icon" onclick="CS.removeItem(\'spotlight.people.items\',' + i + ');window.renderSpotlight()">&#x2715;</button>'
      );
    }).join('');

    document.getElementById('mainArea').innerHTML =
      header('Spotlight', 'Testimonials and the people driving REA.', toolbar('spotlight')) +
      '<div class="panel-body">' +

      card('Teaser card', null,
        '<div class="form-grid" style="grid-template-columns:1fr 1fr;gap:14px;">' +
          field('Section label', input('spotlight.teaser.sectionLabel', teaser.sectionLabel)) +
          field('Headline',      input('spotlight.teaser.headline',     teaser.headline)) +
      ...
      card(
        ...
        '<div class="form-grid" style="grid-template-columns:1fr 1fr;gap:14px;margin-bottom:12px;">' +
          field('Act label', input('spotlight.testimonials.actLabel', tObj.actLabel)) +
          field('Act title', input('spotlight.testimonials.actTitle', tObj.actTitle)) +
          '<div class="field" style="grid-column:span 2">' +
            field('Act description', input('spotlight.testimonials.actDesc', tObj.actDesc)) +
          '</div>' +
        '</div>' +
        testimonialRows +
      ...
      card(
        ...
        e(pObj.actDesc  || ''),
        '<div class="form-grid" style="grid-template-columns:1fr 1fr;gap:14px;margin-bottom:12px;">' +
          field('Act label', input('spotlight.people.actLabel', pObj.actLabel)) +
          field('Act title', input('spotlight.people.actTitle', pObj.actTitle)) +
          '<div class="field" style="grid-column:span 2">' +
            field('Act description', input('spotlight.people.actDesc', pObj.actDesc)) +
          '</div>' +
        '</div>' +
        peopleRows +
  ...
  window._spotlightTestimonialDrawer = function (idx) {
    ...
    document.getElementById('drawerBody').innerHTML =
      '<div class="form-grid" style="grid-template-columns:1fr;gap:12px;">' +
        field('Quote', '<textarea id="tq" rows="3">' + e(t.quote) + '</textarea>') +
  ...
  window._spotlightPersonDrawer = function (idx) {
    ...
    document.getElementById('drawerTitle').textContent    = isNew ? 'Add person' : 'Edit — ' + p.name;
    ...
    document.getElementById('drawerBody').innerHTML =
      '<div class="form-grid" style="grid-template-columns:1fr;gap:12px;">' +
        ...
        field('Contribution', '<textarea id="pc" rows="2">' + e(p.contribution) + '</textarea>') +
        field('Tag',
          '<select id="pt">' +
  ...
  window.renderWhatsNew = function () {
    ...
    var buzzRows = buzz.map(function (b, i) {
      var base = 'whatsNew.buzzItems.' + i;
      return '<div class="section-card" style="margin-bottom:12px;">' +
        '<div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;">' +
          '<span class="pill ' + (b.isNew ? 'pill-deployed' : 'pill-uat') + '" style="font-size:10px;">' + e(b.age) + '</span>' +
          '<span style="font-size:11px;color:var(--txt2);">' + e(b.id) + '</span>' +
    ...
    document.getElementById('mainArea').innerHTML =
      header("What's New", "Editor's pick and buzz feed cards.", toolbar('whatsnew')) +
      '<div class="panel-body">' +

      card('Teaser card', null,
        '<div class="form-grid" style="grid-template-columns:1fr 1fr;gap:14px;">' +
          field('Section label', input('whatsNew.teaser.sectionLabel', teaser.sectionLabel)) +
          field('Headline',      input('whatsNew.teaser.headline',     teaser.headline)) +
      ...
      card("Editor's pick",
        ...
        '<div class="form-grid" style="grid-template-columns:1fr;gap:14px;">' +
          field('Heading', input('whatsNew.editorsPick.heading', pick.heading)) +
      ...
      '<div class="section-card">' +
        '<h4>Buzz items <span style="font-size:11px;color:var(--txt2);font-weight:normal;">' +
          buzz.length + ' of 5 slots</span></h4>' +
